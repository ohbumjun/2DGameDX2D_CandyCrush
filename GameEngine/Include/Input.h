...
struct KeyInfo {
	...
	bool Alt;
	bool Shift;
	std::function<void()> Callback[Key_State::Max];
...
public :
	template<typename T>
	void SetKeyCallback(const std::string& Name, Key_State State, T* Obj, void(T::*Func)())
{
		KeyInfo* Key = FindKeyInfo(Name);
		if (!Key)
			return;
		Key->Callback[(int)State] = std::bind(Func, Obj);
}
private :
