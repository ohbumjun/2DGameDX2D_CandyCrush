...
namespace DirectX
{
	...
	public:
		...
		ScratchImage(ScratchImage&& moveFrom) noexcept :
			...
			m_metadata{},
			m_image(nullptr),
			m_memory(nullptr) { *this = std::move(moveFrom); }
		...
		HRESULT __cdecl Initialize2D(_In_ DXGI_FORMAT fmt, _In_ size_t width, _In_ size_t height, _In_ size_t arraySize,
		                             _In_ size_t mipLevels, _In_ CP_FLAGS flags = CP_FLAGS_NONE) noexcept;
		HRESULT __cdecl Initialize3D(_In_ DXGI_FORMAT fmt, _In_ size_t width, _In_ size_t height, _In_ size_t depth,
		                             _In_ size_t mipLevels, _In_ CP_FLAGS flags = CP_FLAGS_NONE) noexcept;
		HRESULT __cdecl InitializeCube(_In_ DXGI_FORMAT fmt, _In_ size_t width, _In_ size_t height, _In_ size_t nCubes,
		                               _In_ size_t      mipLevels, _In_ CP_FLAGS flags = CP_FLAGS_NONE) noexcept;
	...
	public:
		...
		Blob(Blob&& moveFrom) noexcept :
			m_buffer(nullptr),
			m_size(0) { *this = std::move(moveFrom); }
	...
	HRESULT __cdecl Convert(
		_In_reads_(nimages) const Image* srcImages, _In_ size_t        nimages, _In_ const TexMetadata& metadata,
		_In_ DXGI_FORMAT                 format, _In_ TEX_FILTER_FLAGS filter, _In_ float               threshold,
	...
	HRESULT __cdecl Compress(
		_In_reads_(nimages) const Image* srcImages, _In_ size_t          nimages, _In_ const TexMetadata& metadata,
		_In_ DXGI_FORMAT                 format, _In_ TEX_COMPRESS_FLAGS compress, _In_ float             threshold,
	...
	HRESULT __cdecl Compress(
		_In_ ID3D11Device* pDevice, _In_ const Image& srcImage, _In_ DXGI_FORMAT format,
	...
	HRESULT __cdecl Compress(
		...
		_In_ DXGI_FORMAT    format, _In_ TEX_COMPRESS_FLAGS compress, _In_ float alphaWeight,
		_Out_ ScratchImage& cImages) noexcept;
	...
	HRESULT __cdecl CreateTexture(
		_In_ ID3D11Device*        pDevice, _In_reads_(nimages) const Image* srcImages, _In_ size_t nimages,
